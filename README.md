...
*   **启动脚本**: `MT-NUIST.py`
*   **简介**: 基于 ASE (Atomic Simulation Environment) 开发，主要用于原子结构建模、VASP 远程任务提交与监控、以及能带结构可视化。
...
*   **启动脚本**: `CHGnet_reload.py`
*   **简介**: 基于 CHGNet (Crystal Hamiltonian Graph Neural Network) 开发，利用预训练的通用机器学习势能进行快速的材料模拟。
...
5.  **附属模块**: VASP 工具代码中引用了 `CIFDownload.py` 和 `DescriptorDesign.py` 模块，请确保它们在同一目录下，否则相关菜单功能将不可用。
